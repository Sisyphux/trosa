-- Keep Customer priority and file metadata in canonical relations.
-- Compatibility rows retain only the integer HTTP identifier and storage
-- adapter fields required by older clients.
BEGIN;

ALTER TABLE core.file_objects ADD COLUMN IF NOT EXISTS category text NOT NULL DEFAULT '';
ALTER TABLE core.file_objects ADD COLUMN IF NOT EXISTS uploaded_by text NOT NULL DEFAULT '';
...
SELECT ref.legacy_customer_id AS id, a.id AS account_id,
       ...
       trosa.compat_local_date(a.last_contact_at) AS last_interaction_on,
       trosa.compat_local_date(a.next_follow_up_at) AS next_task_on,
       trosa.compat_local_date(d.pinned_at) AS pinned_at
