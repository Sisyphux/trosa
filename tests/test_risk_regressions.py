...
class InputBoundaryRegressionTest(unittest.TestCase):
    ...
    def test_customer_details_are_a_formal_postgres_fact_and_compat_writes_sync_them(self):
        ...
        self.assertIn('INSERT INTO trosa.customer_details', migration)
        self.assertIn('ON CONFLICT (account_id) DO UPDATE', migration)
        self.assertIn('manual_next_task = excluded.manual_next_task', migration)
        self.assertIn('Product readers never need legacy_payload', migration)
        self.assertIn('"trosa.customer_details"', (ROOT / 'postgres_schema_contract.py').read_text(encoding='utf-8'))
        projection = (ROOT / 'migrations' / '0024_customer_record_task_projection.sql').read_text(encoding='utf-8')
        self.assertIn('coalesce(d.manual_next_task, false) AS manual_next_task', projection)
