...
> 更新：2026-09-12。依据当前仓库、ECS 只读运行链路、正式 PostgreSQL 运行契约和本地真实 PostgreSQL 演练整理。本文是维护入口，不是重构方案；正式发布后的远端版本与数据库账本仍以 `status-workbench.sh` 和受控备份/核验记录为准。本轮功能收敛 commit `7535b79` 已发布，当前线上 release 为 `auto-20260912083340-d1d6972`（最终文档基线）。
...
- 2026-09-12 发布后只读检查：`trade-os=active/running`、`cloudflared=active`，当前 release 为 `auto-20260912083340-d1d6972`；公网 ping 返回 `status=ok`、`backend=postgresql`、`formal_runtime=true`、`runtime_contract=trosa-postgresql-v1`。受控 `verify_schema` 已确认 0001–0029 全部通过、schema contract 完整、两个 legacy reference orphan count 均为 0。
