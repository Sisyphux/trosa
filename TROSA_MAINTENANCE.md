...
> 更新：2026-09-12。依据当前仓库、ECS 只读运行链路、正式 PostgreSQL 运行契约和本地真实 PostgreSQL 演练整理。本文是维护入口，不是重构方案；正式发布后的远端版本与数据库账本仍以 `status-workbench.sh` 和受控备份/核验记录为准。本轮功能收敛 commit `7535b79` 已发布，最近一次运行代码 release 为 `auto-20260912085022-9d7d87b`（commit `9d7d87b`）；当前 ECS release 以状态脚本实时输出为准。
...
- 2026-09-12 发布后只读检查：`trade-os=active/running`、`cloudflared=active`，最近一次运行代码 release 为 `auto-20260912085022-9d7d87b`；公网 ping 返回 `status=ok`、`backend=postgresql`、`formal_runtime=true`、`runtime_contract=trosa-postgresql-v1`。受控 `verify_schema` 已确认当时全部正式迁移通过、schema contract 完整、两个 legacy reference orphan count 均为 0（迁移编号持续增长，最新范围以 `migrations/` 目录和 ECS 账本实时输出为准，不要在本页写死编号）。
- 普通代码任务先在独立 worktree 中提交为逻辑完整的 commit，再由 `deploy/cloud/auto-publish.sh --commit ...` 或 `--branch ...` 构建干净 release worktree、跑完整门禁、推送 release commit 到 `main`，再由 `trosa-release publish` 让 ECS 后台任务拉取并按阶段发布。调用者的 dirty working tree 不参与发布；当前 ECS release 以 `deploy/cloud/trosa-release status --json` 的实时输出为准。有数据库变化的改动自动先做服务端预迁移备份；疑似破坏性迁移需明确确认（`--allow-destructive-db`）。开发/审查 Agent 会话设置 `TRADE_OS_AGENT_ROLE=dev`（或 `review`），只有 release 角色能执行 publish/rollback；发布配置 `workbench.env` 位于仓库外（`~/.config/trosa/workbench.env`）。每次 `test`/`publish` 的完成证据写入 `trosa-tasks/<id>.verify.log`，任务清单 `status=landed` 才算完成。
