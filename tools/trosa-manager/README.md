...
服务器工作台管理的是服务器、本机文件、备份和发布流程；**Trosa 是目前运行在这台服务器上的项目**。GitHub 仓库保存的是 Trosa 源码，不是服务器工作台本身。
...
`deploy/cloud/workbench.env` 的 Trosa 项目目录。首次打开“文件与资料”时，macOS 可能要求一次性允许访问桌面文件夹；这是读取项目和你主动选择上传文件所必需的系统权限。

## 当前功能

- 概览首页直接显示服务器、Trosa 网站、Trosa 应用、安全连接和当前网站版本是否正常；同时显示 CPU、内存、磁盘、负载和运行时间，每 30 秒自动刷新
- 当 SSH 不可用时，状态读取会通过已开启的 ECS Session Manager（SSM）交互式会话完成；页面仍只显示状态结果，不开放公网管理端口
