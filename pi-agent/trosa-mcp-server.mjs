...
server.registerTool("resolve_inbox", { description: "Resolve a known open Inbox item reversibly.", inputSchema: { inbox_item_id: z.number().int().positive(), resolution_note: z.string().max(1000).optional(), ...writeSchema } },
  (args) => callGateway("/api/gateway/actions", { method: "POST", idempotencyKey: writeKey(args.idempotency_key), body: { action: "resolve_inbox", payload: { inbox_item_id: args.inbox_item_id, resolution_note: args.resolution_note || "", source: "trosa_mcp" } } }));
server.registerTool("assign_inbox_customer", { description: "Confirm a known unassigned Inbox communication belongs to a known customer. It keeps the item open for the later communication record.", inputSchema: { inbox_item_id: z.number().int().positive(), customer_id: z.number().int().positive(), ...writeSchema } },
  (args) => callGateway("/api/gateway/actions", { method: "POST", idempotencyKey: writeKey(args.idempotency_key), body: { action: "assign_inbox_customer", customer_id: args.customer_id, payload: { inbox_item_id: args.inbox_item_id, source: "trosa_mcp" } } }));
